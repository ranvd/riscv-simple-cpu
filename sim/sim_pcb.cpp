...
void Vclocks(VVpcb *top, VerilatedVcdC *tfp, int n) {
    int counter = 0;
    for (int i = 0; i < n; i++) {
        top->clk_i = 0;
        top->eval();
        ...
        if (anomaly) {
            ...
            if (anomaly > 4) {
                anomaly = 0;
                top->Vpcb->Core1->regfile1->readReg(10, a0);
                if (a0) break;
            }
        }
...
int main(int argc, char **argv) {
    ...
    sim_mem_load_bin(top->Vpcb->Core1->IF1->instr_cache1, std::string(argv[1]));
    sim_mem_load_bin(top->Vpcb->Core1->MEM1->data_cache1, std::string(argv[1]));
